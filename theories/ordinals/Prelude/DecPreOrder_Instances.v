...
Instance Zle_dec : RelDecision Z.le := Z_le_dec.
...
Global Instance To_lex_prod `{!RelDecision leA} `{!RelDecision leB} :
 TotalPreOrder lex_prod.
Proof.
  split.
  - apply PO_lex_prod.
...
Global Instance lex_prod_dec `{!RelDecision leA} `{!RelDecision leB} :
 RelDecision lex_prod.
Proof.
intros x y; destruct x, y.
